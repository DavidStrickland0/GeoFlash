...
{
    class MultipleChoiceCapitolViewModel:GeoFlashViewModel,INotifyPropertyChanged 
    {

        public MultipleChoiceCapitolViewModel()
            : base(false,true)
        {
        }
        ...
        protected override void UpdateCard()
        {
            ...
            answerList.Add(rnd.Next(), CardList[CurrentIndex].ImageCapitol);
            while (answerList.Count < 4)
            {
                string nextPossibleName = CardList[rnd.Next(CardList.Count)].ImageCapitol;
        ...
        internal void AnswerSelected(int selected)
        {
            ...
            string correctAnswer = CardList[CurrentIndex].ImageCapitol;
            switch (selected)
            {
                case 1:
